...
@Component({
  ...
  standalone: true,
  imports: [NgFor, MatCardModule, MatRippleModule, NgIf, MatPaginatorModule]
})
export class SearchResourcesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get(this.apiUrl + '/api/public_question_sets/?page_size=' + this.pageSize).subscribe({
      next: (data: any) => {
  ...
  handlePageEvent(e: PageEvent) {
    ...
    this.http.get(this.apiUrl + '/api/public_question_sets/?page=' + query_idx + '&' + 'page_size=' + this.pageSize).subscribe({
      next: (data: any) => {
        this.resources = data.results;
      }
    })
